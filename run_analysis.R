...
# Set Work Directory
setwd("~/Data Scientist/Course 3 - Getting and Cleaning Data/Project")
...
zip_file <- "projectfiles.zip" 
download.file(url, zip_file)

#unzip
unzip(zip_file, list = FALSE, overwrite = TRUE) 
...
colnames(alldata) <- c("id", "activity", gsub('[-()]', '', features$V2)) #gsub makes colnames tidy

#View(alldata)

# 2) Extracts only the measurements on the mean and standard deviation for each measurement.
mean_std <- alldata[,c(1,2,grep("mean",colnames(alldata)),grep("std",colnames(alldata)))] %>%
  arrange(id, activity)
